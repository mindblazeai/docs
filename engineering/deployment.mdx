...
## Deploying to staging Write two short sentences on how to roll back a bad production deploy
...
If a deploy goes wrong, redeploy the previous release tag and re-run smoke tests.
...
Keep the previous release tag handy so a rollback is one command away.
...
Verifying your changes in staging is the best way to prevent a production rollback. If a bad deploy does make it to production, see the Rollbacks section for the emergency procedure.

To roll back a bad production deploy, use the deploy tool to redeploy the previous successful build. This should always be done first, before attempting to diagnose the problem.

To roll back a bad production deploy, find the previous successful build in the deploy tool and promote it to production again. This action quickly replaces the faulty deployment with a known good version.
